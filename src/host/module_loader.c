...
static char *joinPath(const char *left, const char *right) {
  ...
  bool hasSeparator = leftLength > 0 && (left[leftLength - 1] == '/' || left[leftLength - 1] == '\\');
  ...
  if (!hasSeparator) path[leftLength] = '/';
...
static char *executableDirectory(void) {
  ...
  if (length == 0 || length >= sizeof(path)) return NULL;
#else
  char path[4096];
  ...
  if (slash == NULL) return copyText(".", 1);
  if (slash == path) return copyText(path, 1);
  return copyText(path, (size_t)(slash - path));
}

static char *standardLibraryRoot(void) {
  const char *overridePath = getenv("PB_STDLIB_PATH");
  if (overridePath != NULL && overridePath[0] != '\0') return copyText(overridePath, strlen(overridePath));
  ...
  if (executableRoot != NULL) {
    ...
    free(executableRoot);
    if (developmentRoot != NULL && isDirectory(developmentRoot)) return developmentRoot;
...
bool initModuleLoader(ModuleLoader *loader, const char *entryPath) {
  ...
  if (loader->loadedProviders == NULL) return false;
  ...
  if (entryPath == NULL) {
    ...
    if (loader->root != NULL) return true;
...
void freeModuleLoader(ModuleLoader *loader) {
  for (size_t i = loader->providerCount; i > 0; i--) {
    size_t index = i - 1;
    if (loader->loadedProviders[index] && providers[index].unload != NULL) providers[index].unload();
  }
  free(loader->loadedProviders);
  free(loader->root);
  free(loader->standardRoot);
...
static bool loadProvidedModule(ModuleLoader *loader, PbVM *vm, const char *name, bool *matched) {
...
static bool validModuleName(const char *name) {
  ...
  for (const char *current = name;; current++) {
    ...
    if (character != '\0' && character != '/' && character != '_' && character != '-' && character != '.' &&
        !isalnum(character))
      return false;
...
static char *modulePath(const char *root, PbVM *vm, const char *name) {
  ...
  snprintf(path, rootLength + nameLength + 5, "%s/%s.pb", root, name);
  return path;
}
...
static char *readModule(PbVM *vm, const char *name, const char *path) {
  ...
  if (file == NULL) {
    ...
    if (errno == ENOENT)
      ...
      snprintf(message, sizeof(message), "Could not open module file '%s'.", path);
    pbRuntimeError(vm, message);
    return NULL;
  }
...
static bool loadSourceModule(ModuleLoader *loader, PbVM *vm, const char *name) {
