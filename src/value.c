...
static bool hashmapsEqual(ObjHashmap *left, ObjHashmap *right, EqualityContext *context) {
  ...
  bool equal = true;
  for (int index = mapFirstEntry(&left->items); index != -1; index = mapNextEntry(&left->items, index)) {
    MapEntry *entry = mapEntryAt(&left->items, index);
    Value rightValue;
    if (!mapGet(&right->items, entry->key, &rightValue) || !valuesEqualInternal(entry->value, rightValue, context)) {
      equal = false;
      break;
    }
  }
