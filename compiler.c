...
static void advance() {
  ...
  for (;;) {
    ...
    if (parser.current.type != TOKEN_ERROR) break; // end the loop if we see a non-error token or if we reach the end of file
...
static void initCompiler(Compiler* compiler, FunctionType type) {
  compiler->enclosing = current; // capture the about to no longer be the current compiler for our stack(linked list) of compilers
