...
int main(void) {
  ...
  while (!WindowShouldClose()) {
    if (statusMessageTimer > 0) statusMessageTimer -= GetFrameTime();
    ...
    if (mode == MODE_EDIT) {
      ...
      if (IsKeyDown(KEY_LEFT_CONTROL)) {
        if (IsKeyPressed(KEY_S)) {
          ...
          if (HasSelection(&textBuffer)) SetClipboardText(GetSelection(&textBuffer));
          ...
          if (terminalCmd != NULL) {
            if (strcmp(terminalCmd, "gnome-terminal") == 0) {
              ...
              sprintf(command,
                      "gnome-terminal -- sh -c 'cd files/build && LD_LIBRARY_PATH=lib ./pb ../%s; echo; read -p "
                      "\"Process finished. Press Enter to exit.\"'",
                      currentFilename);
            } else {
              // Hardcoded command for other terminals like xterm
              sprintf(command,
                      "%s -e \"sh -c 'cd files/build && LD_LIBRARY_PATH=lib ./pb ../%s; echo; read -p \\\"Process "
                      "finished. Press Enter to exit.\\\"'\"",
                      terminalCmd, currentFilename);
            }
        ...
        if (IsKeyPressed(KEY_LEFT) || IsKeyPressedRepeat(KEY_LEFT)) {
          if (!shift && HasSelection(&textBuffer))
            textBuffer.cursor = (textBuffer.selectionStart < textBuffer.selectionEnd) ? textBuffer.selectionStart
                                                                                      : textBuffer.selectionEnd;
        ...
        if (selectionChangedByKey) {
          if (shift) {
            if (textBuffer.selectionStart == textBuffer.selectionEnd) textBuffer.selectionStart = textBuffer.cursor;
      ...
      if (IsKeyPressed(KEY_UP)) fileSelection--;
      if (IsKeyPressed(KEY_DOWN)) fileSelection++;
      if (fileSelection < 0) fileSelection = 0;
      if (fileSelection >= (int)fileList.count) fileSelection = fileList.count - 1;
    ...
    if (mode == MODE_EDIT) {
      ...
      for (int i = 0; i <= textBuffer.length; i++) {
        if (textBuffer.buffer[i] == '\n' || textBuffer.buffer[i] == '\0') {
          ...
          if (HasSelection(&textBuffer)) {
            int selStart = (textBuffer.selectionStart < textBuffer.selectionEnd) ? textBuffer.selectionStart
                                                                                 : textBuffer.selectionEnd;
            int selEnd = (textBuffer.selectionStart > textBuffer.selectionEnd) ? textBuffer.selectionStart
                                                                               : textBuffer.selectionEnd;
            ...
            if (selStart < lineStartIdx + lineLength && selEnd > lineStartIdx) {
              ...
              if (selStart > lineStartIdx) {
                startX =
                    MeasureTextEx(font, TextSubtext(currentLineStart, 0, selStart - lineStartIdx), EDITOR_FONT_SIZE, 1)
                        .x;
              }
              int endX =
                  MeasureTextEx(font,
                                TextSubtext(currentLineStart, 0,
                                            (selEnd > lineStartIdx + lineLength) ? lineLength : selEnd - lineStartIdx),
                                EDITOR_FONT_SIZE, 1)
                      .x;
              DrawRectangle(gutterWidth + 10 + startX, textY, endX - startX, EDITOR_FONT_SIZE * EDITOR_LINE_SPACING,
                            selectionColor);
            }
          }
      ...
      for (unsigned int i = 0; i < fileList.count; i++) {
        ...
        if (TextIsEqual(GetFileExtension(filename), ".pb")) {
          DrawText(filename, menuPos.x + 20, menuPos.y + 40 + i * 30, 20,
                   (i == fileSelection) ? (Color){255, 255, 0, 255} : textColor);
        }
      }
    } else if (mode == MODE_SAVE_AS) {
      DrawRectangle(0, 0, GetScreenWidth(), GetScreenHeight(), Fade(bgColor, 0.8f));
...
void LoadFile(TextBuffer *tb, const char *filename) {
  ...
  if (FileExists(path) && strcmp(filename, "untitled.pb") != 0) {
    ...
    const char *initialText =
        "-- welcome to pogberry --\n\n- this file will be overwritten, so make sure to press ctrl+s to save a new file "
        "before writing any code \n- or ctrl+o to open an existing file\n- you can run the code by pressing ctrl+i or "
        "save it as an executable using ctrl+r\n -basic shortcuts such as ctrl + a, c, z etc do work but you cannot "
        "select text right now\n- enjoyyyy";
    FreeTextBuffer(tb);
    InitTextBuffer(tb);
...
void NewFile(TextBuffer *tb) { LoadFile(tb, "untitled.pb"); }
...
void InsertChar(TextBuffer *tb, char c) {
  ...
  if (tb->cursor < tb->length) memmove(&tb->buffer[tb->cursor + 1], &tb->buffer[tb->cursor], tb->length - tb->cursor);
...
void DeleteSelection(TextBuffer *tb) {
  ...
  int len = end - start;
  if (end < tb->length) memmove(&tb->buffer[start], &tb->buffer[end], tb->length - end);
  tb->length -= len;
  tb->cursor = start;
  tb->buffer[tb->length] = '\0';
  ClearSelection(tb);
...
bool HasSelection(TextBuffer *tb) { return tb->selectionStart != tb->selectionEnd; }
...
const char *FindBestTerminal(void) {
  ...
  const char *terminalList[] = {"gnome-terminal", "konsole", "xfce4-terminal", "xterm", NULL};
